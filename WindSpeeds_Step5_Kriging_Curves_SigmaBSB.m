...
load('Cross-semivariogramSigmaBSBProfiles.mat','fitprof')
...
for p = 1:n
    tic
    for m=1:12
        krigged_val(p,m) = cokrigingVueltaTuercaNegCorrected(p,m,edges, newlat, newlon, lats, lons, D, Dp(p,:)', profile.(f), g.(f), fitprof.(f));
    end 
    toc
%     if p==1000
...
save('CokrigingResults_SigmaBSB_NegCorrected.mat','newlat','newlon','profile','krigged_profile','krigged_total')
...
figure
scatter(newlon,newlat, 30, krigged_profile.hour23(:,1), 'filled');
