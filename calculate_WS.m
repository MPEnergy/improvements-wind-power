function newWS = calculate_WS(thisLat, thisLon, allLat, allLon, params, dates, ws, withError, hoursAhead, randGenerator)
...
if hoursAhead<24
%     [betas, noiseVarianceMultiplier] = local_checkInvertibilityARMA(alpha1, alpha2, betas(1:hoursAhead-1));
...
% % % % % % epsilon(2) = epsilon(1);
% % % % % % epsilontilde(2) = epsilontilde(1);
% % % % % % sigma_GARCH(2) = sqrt(omega+(gamma0+gamma1*(epsilon(1)>0))*epsilon(1)^2+delta*sigma_GARCH(1)^2);
% if hoursAhead<2
%     hoursAhead = 2;
% %     epsilon(2) = epsilon(1);
% %     epsilontilde(2) = epsilontilde(1);
...
%WS(2) = S(2) + alpha1*(WS(1)-S(1))+epsilonhat(2)+betas(1)*epsilonhat(1);
...
epsilonhat(t-hoursAhead) = WS(t-hoursAhead)-ws(t-hoursAhead);
epsilontilde(t-hoursAhead) = epsilonhat(t-hoursAhead)/sqrt(sigma_BSB(t-hoursAhead));
if t-hoursAhead-1>1
    epsilon(t-hoursAhead) = epsilontilde(t-hoursAhead)-lambda*epsilontilde(t-hoursAhead-1);
    sigma_GARCH(t-hoursAhead) = sqrt(omega+(gamma0+gamma1*(epsilon(t-hoursAhead-1)>0))*epsilon(t-hoursAhead-1)^2+delta*sigma_GARCH(t-hoursAhead-1)^2);
else
    epsilon(t-hoursAhead) = epsilontilde(t-hoursAhead);
    sigma_GARCH(t-hoursAhead) = sqrt(omega);
